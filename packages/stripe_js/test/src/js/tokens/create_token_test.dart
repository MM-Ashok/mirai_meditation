...
void main() {
  group('createToken', () {
    ...
    group('createBankAccountToken', () {
      test('can be called', () async {
        expect(
          stripe.createBankAccountToken(const CreateTokenBankAccountData(
            country: 'US',
            currency: 'USD',
            accountNumber: '22',
          )),
      ...
      test('returns  when invalid data', () async {
        final response =
            await stripe.createBankAccountToken(const CreateTokenBankAccountData(
          country: 'invalid',
          currency: 'USD',
          accountNumber: '22',
        ));
    ...
    group('createPIIToken', () {
      test('can be called', () async {
        expect(
          stripe.createPIIToken(const CreateTokenPIIData(personalIdNumber: '124')),
          completes,
        );
      });
      ...
      test('returns new token', () async {
        final response = await stripe.createPIIToken(
          const CreateTokenPIIData(personalIdNumber: 'a'),
        );
        expect(
          response.token?.id,
          startsWith('pii_'),
